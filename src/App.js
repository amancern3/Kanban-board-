...
class App extends Component {
  ...
  handleAdd = columnIndex => {
    const name = window.prompt('Add Card')
    if(!name) return
    const card = { name }
  ...
  handleRemove = columnIndex => {
    const name = window.prompt('Remove what Card ?')
    if (!name) return
    const card = { name }
    this.setState(prevState => {
      const {columns} = this.state
      columns[columnIndex].cards.pop(card)
      return {columns}
    })
  }
  render() {
      ...
      <div className="App">
        {this.state.columns.map((column, columnIndex) => (
          <Column
            ...
            onRemoveCard={() => this.handleRemove(columnIndex)}
          />
        ))}
      </div>
    );
